...
class UsuariosModel extends Conexion{
	...
	public function registarUsuariosModelo($datos){
		$stmt = Conexion::conectar()->prepare("INSERT INTO personas (`nombre`, `apellido`, `documentoIdentidad`, `numeroDocumento`, `fechaNacimiento`) VALUES (?,?,?,?,?)");
	...
	public function actualizarUsuarioModel($datos){
		$stmt = conexion::conectar()->prepare("UPDATE personas SET nombre=?,apellido=?,documentoIdentidad=?,numeroDocumento=?,fechaNacimiento=? WHERE idPersonas = ?");
	...
	public function actualizarPerfilModel($datos, $tabla){
		$stmt = conexion::conectar()->prepare("UPDATE $tabla SET nombre = :nombre, fechaNacimiento = :fN, apellido = :apellido WHERE idPersonas = :id");
